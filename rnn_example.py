...
class RNN(object):
    ...
    def __init__(self, input, n_in, n_hidden, n_out, activation=T.tanh,
        ...
        if self.output_type == 'real':
            ...
            self.p_y_given_x = self.softmax(self.y_pred)   # DS:  Relevant

            # compute prediction as class whose probability is maximal
            self.y_out = T.argmax(self.p_y_given_x, axis=-1)  # DS:  Relevant
...
class MetaRNN(BaseEstimator):
    ...
    def fit(self, X_train, Y_train, X_test=None, Y_test=None,
        ...
        # Search "loss defined" to fine loss
        
        #return -T.mean(T.log(self.p_y_given_x)[T.arange(y.shape[0]), y])
        
        
        cost = self.rnn.loss(self.y) \  # Their loss function
            + self.L1_reg * self.rnn.L1 \
            + self.L2_reg * self.rnn.L2_sqr

        # DS Note:  self.y is a theano matrix
        ...
        while (epoch < self.n_epochs):
            ...
            for idx in xrange(n_train):
                ...
                example_cost = train_model(idx, self.learning_rate,
                                           effective_momentum)  # DS:  Relevant
                ...
                if iter % validation_frequency == 0:
                    ...
                    train_losses = [compute_train_error(i)  # DS:  Could be relevant
