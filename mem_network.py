...
# The incomings are (l_context_in, l_B_embedding, l_context_pe_in)
# In model they are defined as:
...
class MemoryNetworkLayer(lasagne.layers.MergeLayer):
    ...
    def __init__(self, incomings, vocab, embedding_size, A, A_T, C, C_T, nonlinearity=lasagne.nonlinearities.softmax, **kwargs):
        ...
        l_context_pe_in = lasagne.layers.InputLayer(shape=(batch_size, max_seqlen, max_sentlen, embedding_size))  # position_encoding
        ...
        self.A_T = l_A_embedding.T  # I believe the T stands for the temporal encoding
        ...
        l_prob = InnerProductLayer((l_A_embedding, l_B_embedding), nonlinearity=nonlinearity)  # DS Note:  Eq. 1 from the paper
...
class Model:
    ...
    def __init__(self, train_file, test_file, data_source='babi', batch_size=4, embedding_size=20, max_norm=40, lr=0.01, num_hops=3, adj_weight_tying=True, linear_start=True, **kwargs):
        ...
        #     print(" pre shape: ", S[i].shape)
        #     print(" shape: ", S[i].reshape((-1, max_sentlen)).shape)  # In babi, this is (1,7) or (1, max_sentlen)

        # for row in self.data['train']['C']:
        ...
        lb = LabelBinarizer()
        lb.fit(list(vocab))
        vocab = lb.classes_.tolist()  # Just the words in a list
    ...
    def build_network(self, nonlinearity):
        ...
        self.a_shared = theano.shared(np.zeros((batch_size, self.num_classes), dtype=np.int32), borrow=True)  # DS Note: Num classes is 21, length of the vocab + 1, this means how many answers it could be
        self.c_pe_shared = theano.shared(np.zeros((batch_size, max_seqlen, max_sentlen, embedding_size), dtype=theano.config.floatX), borrow=True)  # embedding size is 20
    ...
    def compute_f1(self, dataset):
        assert(len(dataset['Y']) > self.batch_size)
    ...
    def set_shared_variables(self, dataset, index):
        ...
        #print(" size c_pe: ", c_pe.shape)
        #print(" size q_qe: ", q_pe.shape)

        indices = range(index*self.batch_size, (index+1)*self.batch_size)  #indices of this batch
        ...
        for key, mask in [('C', c_pe), ('Q', q_pe)]:
            ...
            # Note that sometimes row is a single idx, other times it is a list of indices
            # The size of row will be the number of the sentences relevant to that article
            for i, row in enumerate(dataset[key][indices]):  # iterates through sentences in batch
                #print(" row: ", row)
                #print(" at 417 shape: ", self.S[row].reshape((-1, self.max_sentlen)).shape)
                sentences = self.S[row].reshape((-1, self.max_sentlen))  # -1 means the value is inferred based on max_sent_len
                #print(" len of 0 : ", len(sentences[0]))
                #print(" len sentences: ", len(sentences))
                #print(" sentences: ", sentences)
                # note:  sentences is a list of arrays of the word2idxs.  It is the sentences in the article
                # word_idxs is a list of word_idxs in that sentence
                #print(" sentences:  ", sentences)

                for ii, word_idxs in enumerate(sentences):
                    #print(" word idx: ", word_idxs)
                    J = np.count_nonzero(word_idxs)
                    #print(" J: ", J)
                    for j in np.arange(J):
                        #print(" self embedding size: ", self.embedding_size)
                        #print(" cur i: ", i, " ii: ", ii, " j: ", j)
    ...
    def get_vocab(self, lines):
        ...
        for i, line in enumerate(lines):
            if line['type'] == 'q':
                ...
                # DS Note:  believe it's the number of sentences , as you check whether the type is a sentence
                max_seqlen = max(len(indices), max_seqlen)

        return vocab, word_to_idx, idx_to_word, max_seqlen, max_sentlen

    # Just creating vectors of the word indices
    # Input offset is 0 for train, and len(S_train) for test
    def process_dataset(self, lines, word_to_idx, max_sentlen, offset):
        ...
        for i, line in enumerate(lines):
            word_indices = []
            #print(" line: ", line)
            word_indices = [word_to_idx[w] for w in nltk.word_tokenize(line['text'])]  # Note: Max sent len could cause problems because right now it's 180
            word_indices += [0] * (max_sentlen - len(word_indices))
            S.append(word_indices)

            # NOTE:  ID counts position of the 'q' relative to the CURRENT article only.  i.e. for babi it only goes up to 15.
    ...
    # For each line, either creates a sentence type, or a question type
    def get_lines(self, fname):
        ...
        for i, line in enumerate(open(fname)):
            ...
            if line.find('?') == -1:
                ...
                lines.append({'id': id, 'type': 'q', 'text': line[:idx], 'answer': tmp[1].strip(), 'refs': [int(x)-1 for x in tmp[2:][0].split(' ')]})  # Question is sentence question, answer is one word answer
            if False and i > 1000:
                break
...
def main():
    ...
    parser.add_argument('--n_epochs', type=int, default=300, help='Num epochs')
